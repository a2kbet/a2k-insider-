...
const MajorAdvantages = () => {
  return (
    <div className="py-12 px-6 bg-white text-black">
      {/* Page Heading */}
      <h2 className="text-center text-5xl md:text-6xl font-bold mb-10 text-black">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-8xl mx-auto">
        {advantages.map((adv, index) => (
          <div
            key={index}
            className="flex flex-col items-start py-5 px-5 rounded-xl border border-black/10 transition"
          >
            <div className="flex items-center space-x-3 mb-2">
              <div className="text-4xl text-black">{adv.icon}</div>
              <h2 className="text-2xl font-semibold text-black">{adv.title}</h2>
            </div>
            <p className="text-black/70 mt-4 text-lg">{adv.desc}</p>
          </div>
        ))}
      </div>
    </div>
